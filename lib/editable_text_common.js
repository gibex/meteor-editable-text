...
// @todo - on ipad space single space is different
...
Meteor.methods({
  _editableTextWrite : function(action,data,modifier,transaction) {
    ...
    check(data.checklistId, Match.OneOf(String,Number));
    check(data.contentItemId, Match.OneOf(String,undefined));
    ...
    // not a collection (custom code)
    if (data.collection === 'this' && Collection === undefined) {
      switch(data.type) {
        case 'header': ChecklistTemplates.update({_id: data.checklistId}, {$set: {name: modifier["$set"][data.field]}}); 
                       break;
        case 'checkName': ChecklistTemplates.update({_id: data.checklistId, content: {$elemMatch: {_id: data.contentItemId}}}, 
                            {$set: {"content.$.itemName": modifier["$set"][data.field]}});
                       break;
        case 'checkNote': ChecklistTemplates.update({_id: data.checklistId, content: {$elemMatch: {_id: data.contentItemId}}}, 
                            {$set: {"content.$.itemNote": modifier["$set"][data.field]}});
                       break;
        case 'checkIndent': var indent = parseInt(modifier["$set"][data.field]) ? parseInt(modifier["$set"][data.field]) : 0;  
                            if (indent < 0 || indent > 3) indent = 0;
                            ChecklistTemplates.update({_id: data.checklistId, content: {$elemMatch: {_id: data.contentItemId}}}, 
                            {$set: {"content.$.indentLevel": indent}});
                       break;
        case 'checkWebhook': ChecklistTemplates.update({_id: data.checklistId, content: {$elemMatch: {_id: data.contentItemId}}}, 
                            {$set: {"content.$.itemWebhook": modifier["$set"][data.field]}});
                       break;
        case 'checkWebhookValues': ChecklistTemplates.update({_id: data.checklistId, content: {$elemMatch: {_id: data.contentItemId}}}, 
                                  {$set: {"content.$.itemWebhookValues": modifier["$set"][data.field]}});
                        break;                                                    
      }
    } else {
      // collection (vanilla code)
  ...
  _editableCheckbox : function(data,modifier) {
    check(data,Object);
    check(data.collection,String);
    check(data.context,(typeof FS !== "undefined" && FS.File) ? Match.OneOf(Object, FS.File) : Object);
    ...
    if (data.collection === 'this' && Collection === undefined) {
      switch(data.type) {
        case 'webhookEnable': ChecklistTemplates.update({_id: data.checklistId, content: {$elemMatch: {_id: data.contentItemId}}}, 
                              {$set: {"content.$.itemWebhookEnable": modifier["$set"][data.field]}})
                       break;                             
      }
    }
  }  
