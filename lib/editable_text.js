...
EditableText.update = function(Collection,doc,modifier) {
  ...
  // console.log(this.type + ' ' + this.checklistId + ' ' + this.contentItemId);  
  if (EditableText.useMethods && this.useMethod !== false || this.useMethod) {
    Meteor.call('_editableTextWrite','update',this,modifier,EditableText._useTransaction(this), 
      function(err,res) {
        if (err) {
          console.log(err);
        }
...
Template.editable_text_widget.helpers({
  ...
  //   if (this.convertLinks) value = convertToLinks(value.trim());
  ...
  wysiwygContent : function () {
    ...
    var value = this.value || EditableText._drillDown(this.context, this.field);
    return (Template.instance().editHTML.get()) ? value || "" : value && 
          new Spacebars.SafeString(value.replace(/\n/g, '<br>').replace(/ /g, '&nbsp;')) || "";
  },  
  ...
  content : function() {
    ...
    if (this.preText) return '[ ' + this.preText + ' ' + val + ' ]';
    return val; 
  },
